...
Modify config options at the end of the command, use "path.key=value".
    ...
    parser.add_argument("--logits-out", default=None, type=str)
...
def do_test_logits(cfg, model):
    ...
    for batch in prog_bar:
        for micro_batch in batch:
            ...
            gather_probs = [torch.zeros_like(logits) for _ in range(world_size)]
            dist.all_gather(gather_probs, logits)
            logits = torch.stack(gather_probs, dim=1).flatten(0, 1)
            probs.append(logits.data.cpu())

    result = torch.cat(probs, dim=0).numpy()
    # the last batch maybe padded to be divisible by world_size
    result = result[: len(val_loader.dataset)]

    logger.info("Evaluation logits prediction done")
...
def do_train(cfg, model):
    ...
    lr_scheduler = LRMultiplier(
        ...
        max_iter=len(train_loader) + len(train_loader2),
    )
    ...
    for curr_epoch in range(max_epochs):
        ...
        if curr_epoch == 0:
            ...
            _train_loader = train_loader2
        for curr_iter, batch in enumerate(_train_loader):
            ...
            if total_updates % cfg.train.log_interval == 0:
                ...
                logger.info(
                    f"Epoch [{curr_epoch+1}/{max_epochs}] Iter [{curr_iter+1}/{len(train_loader) + len(train_loader2)}]"
...
def main():
    ...
    if load_from is not None:
        ...
        try:
            ...
            del checkpoint['model']['score.4.weight']
            del checkpoint['model']['score.4.bias']
            load_result = model.load_state_dict(checkpoint["model"], strict=False)
    ...
    elif args.logits_only:
        result = do_test_logits(cfg, model)
        if args.logits_out is not None:
            torch.save(result, args.logits_out)
    else:
        do_train(cfg, model)
