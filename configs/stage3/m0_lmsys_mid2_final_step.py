...
from human_pref.data.processors import ProcessorPAB, ZDProcessorPAB, MIDV2ProcessorPAB
...
model_name_or_path = "uploads/lmsys-gemma-pretrain"
...
# data config
def build_dataset(fold, training, max_length, step=False):
    ...
    if training:
        ...
        dataset0 = LMSYSDataset(
            csv_file="./artifacts/dtrainval_en_part1.parquet",
            query=None,  # f"fold != {fold}", or None
            processor=processor,
            include_swap=True,
            is_parquet=True
        )
        
        dataset = torch.utils.data.ConcatDataset([dataset0])
    else:
        processor = MIDV2ProcessorPAB(
            tokenizer=tokenizer,
            max_length=max_length,
        ...
        dataset = LMSYSDataset(
            csv_file="./artifacts/dtrainval.parquet",
            query=f"fold == {fold}",
            processor=processor,
            is_parquet=True
        )
    if step:
        ...
        dataset0 = LMSYSDataset(
            csv_file="./artifacts/dtrainval_other_df.parquet",
            query=None,  # f"fold != {fold}", or None
            processor=processor,
            include_swap=True,
            is_parquet=True
...
dataloader = dict(
    ...
    train2=L(build_data_loader)(
        dataset=L(build_dataset)(fold=VAL_FOLD, training=True, max_length=4096, step=True),
        batch_size=80,
        num_workers=4,
        training=True,
    ),
...
train = dict(
    ...
    clip_grad=False,
    seed=3,
    load_from="./outputs/stage2/m0_lmsys_mid2_pseudo_final/update_last.pth",
)
