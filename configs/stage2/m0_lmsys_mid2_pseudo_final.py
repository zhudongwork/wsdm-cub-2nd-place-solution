...
from human_pref.data.processors import ProcessorPAB, ZDProcessorPAB, MIDProcessorPAB, MIDV2ProcessorPAB
...
def build_dataset(fold, training, max_length):
    ...
    if training:
        ...
        dataset1 = LMSYSDataset(
            csv_file="./artifacts/pseudo_hf_21k.parquet",
            query=None,
            processor=processor,
            include_swap=True,
            is_parquet=True
        ...
        dataset2 = LMSYSDataset(
            csv_file="./artifacts/pseudo_vllm_v2.parquet",
            query=None,
            processor=processor,
            include_swap=True,
            is_parquet=True
        )
        dataset3 = LMSYSDataset(
            csv_file="./artifacts/pseudo_vllm_v3.parquet",
            query=None,
            processor=processor,
            include_swap=True,
            is_parquet=True
        )  
        dataset4 = LMSYSDataset(
            csv_file="./artifacts/pseudo_vllm_v1.parquet",
            query=None,
            processor=processor,
            include_swap=True,
            is_parquet=True
        )  
        dataset = torch.utils.data.ConcatDataset([dataset4, dataset3, dataset1, dataset2])
    else:
        processor = MIDV2ProcessorPAB(
            tokenizer=tokenizer,
            max_length=max_length,
...
train = dict(
    ...
    clip_grad=False,
    seed=3,
    # load_from="../artifacts/stage2/m0/update_last.pth",
)
