    ...
    for curr_epoch in range(max_epochs):
        ...
        for curr_iter, batch in enumerate(train_loader):
            ...
            if total_updates % cfg.train.checkpoint_interval == 0:
                if total_updates >= (len(train_loader) // 3 * 2):
                    checkpoint_path = (
                    ...
                    if rank == 0:
                        logger.info(f"Iter {total_updates} evaluation result: {eval_result}")
                        torch.save(
                            result,
                            Path(cfg.train.work_dir) / f"result_iter_{total_updates}.pth",
                        )
